...
var _ = Describe("MaintenanceExecutionService", func() {
	var service maintenanceUsecases.MaintenanceExecutionService
	...
	BeforeEach(func() {
		...
		service = maintenanceUsecases.NewMaintenanceExecutionService(mockRepository, mockActivityRepository)
	})
	...
	Context("CreateExecution", func() {
		...
		When("activity does not exist", func() {
			BeforeEach(func() {
				mockActivityRepository.getByIDError = maintenanceUsecases.ErrMaintenanceActivityNotFound
			})
	...
	Context("GetExecution", func() {
		...
		When("execution does not exist", func() {
			BeforeEach(func() {
				mockRepository.getByIDError = maintenanceUsecases.ErrMaintenanceExecutionNotFound
			})

			It("should return ErrMaintenanceExecutionNotFound", func() {
				result, err := service.GetExecution(context.Background(), executionID)
				Expect(err).To(MatchError(maintenanceUsecases.ErrMaintenanceExecutionNotFound))
...
type mockMaintenanceExecutionRepository struct {
	executions              map[string]maintenanceDomain.MaintenanceExecution
	executionsByActivity    map[string][]maintenanceDomain.MaintenanceExecution
...
func newMockMaintenanceExecutionRepository() *mockMaintenanceExecutionRepository {
	return &mockMaintenanceExecutionRepository{
		executions:           make(map[string]maintenanceDomain.MaintenanceExecution),
		executionsByActivity: make(map[string][]maintenanceDomain.MaintenanceExecution),
		totalByActivity:      make(map[string]int),
	}
}

func (m *mockMaintenanceExecutionRepository) Create(ctx context.Context, execution maintenanceDomain.MaintenanceExecution) error {
	m.createCalled = true
...
func (m *mockMaintenanceExecutionRepository) GetByID(ctx context.Context, id shareddomain.ID) (maintenanceDomain.MaintenanceExecution, error) {
	m.getByIDCalled = true
	if m.getByIDError != nil {
		return maintenanceDomain.MaintenanceExecution{}, m.getByIDError
	}
	if execution, ok := m.executions[id.String()]; ok {
		return execution, nil
	}
	return maintenanceDomain.MaintenanceExecution{}, maintenanceUsecases.ErrMaintenanceExecutionNotFound
}

func (m *mockMaintenanceExecutionRepository) FindAllByActivity(ctx context.Context, activityID shareddomain.ID, pagination maintenanceUsecases.Pagination) ([]maintenanceDomain.MaintenanceExecution, int, error) {
	...
	return []maintenanceDomain.MaintenanceExecution{}, 0, nil
}

func (m *mockMaintenanceExecutionRepository) Update(ctx context.Context, execution maintenanceDomain.MaintenanceExecution) error {
	m.updateCalled = true
...
func (m *mockMaintenanceExecutionRepository) FindAllOverdue(ctx context.Context, tenantID shareddomain.ID) ([]maintenanceDomain.MaintenanceExecution, error) {
	return nil, nil
}

func (m *mockMaintenanceExecutionRepository) FindAllDueSoon(ctx context.Context, tenantID shareddomain.ID, days int) ([]maintenanceDomain.MaintenanceExecution, error) {
	return nil, nil
}
