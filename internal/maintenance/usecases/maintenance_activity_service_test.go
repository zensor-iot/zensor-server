...
var _ = Describe("MaintenanceActivityService", func() {
	var service maintenanceUsecases.MaintenanceActivityService
	var mockRepository *mockMaintenanceActivityRepository

	BeforeEach(func() {
		mockRepository = newMockMaintenanceActivityRepository()
		service = maintenanceUsecases.NewMaintenanceActivityService(mockRepository)
	})
	...
	Context("GetActivity", func() {
		...
		When("activity does not exist", func() {
			BeforeEach(func() {
				mockRepository.getByIDError = maintenanceUsecases.ErrMaintenanceActivityNotFound
			})

			It("should return ErrMaintenanceActivityNotFound", func() {
				result, err := service.GetActivity(context.Background(), activityID)
				Expect(err).To(MatchError(maintenanceUsecases.ErrMaintenanceActivityNotFound))
...
type mockMaintenanceActivityRepository struct {
	activities            map[string]maintenanceDomain.MaintenanceActivity
	activitiesByTenant    map[string][]maintenanceDomain.MaintenanceActivity
...
func newMockMaintenanceActivityRepository() *mockMaintenanceActivityRepository {
	return &mockMaintenanceActivityRepository{
		activities:         make(map[string]maintenanceDomain.MaintenanceActivity),
		activitiesByTenant: make(map[string][]maintenanceDomain.MaintenanceActivity),
		totalByTenant:      make(map[string]int),
	}
}

func (m *mockMaintenanceActivityRepository) Create(ctx context.Context, activity maintenanceDomain.MaintenanceActivity) error {
	m.createCalled = true
...
func (m *mockMaintenanceActivityRepository) GetByID(ctx context.Context, id shareddomain.ID) (maintenanceDomain.MaintenanceActivity, error) {
	m.getByIDCalled = true
	if m.getByIDError != nil {
		return maintenanceDomain.MaintenanceActivity{}, m.getByIDError
	}
	if activity, ok := m.activities[id.String()]; ok {
		return activity, nil
	}
	return maintenanceDomain.MaintenanceActivity{}, maintenanceUsecases.ErrMaintenanceActivityNotFound
}

func (m *mockMaintenanceActivityRepository) FindAllByTenant(ctx context.Context, tenantID shareddomain.ID, pagination maintenanceUsecases.Pagination) ([]maintenanceDomain.MaintenanceActivity, int, error) {
	...
	return []maintenanceDomain.MaintenanceActivity{}, 0, nil
}

func (m *mockMaintenanceActivityRepository) Update(ctx context.Context, activity maintenanceDomain.MaintenanceActivity) error {
	m.updateCalled = true
