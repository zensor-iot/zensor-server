...
var _ = Describe("MaintenanceExecutionController", func() {
	var controller *maintenance_httpapi.MaintenanceExecutionController
	var mockExecutionService *mockusecases.MockMaintenanceExecutionService
	var mockActivityService *mockusecases.MockMaintenanceActivityService
	...
	BeforeEach(func() {
		...
		mockExecutionService = mockusecases.NewMockMaintenanceExecutionService(ctrl)
		mockActivityService = mockusecases.NewMockMaintenanceActivityService(ctrl)
		controller = maintenance_httpapi.NewMaintenanceExecutionController(mockExecutionService, mockActivityService)
	...
	Context("getExecution", func() {
		...
		When("execution not found", func() {
			...
			It("should return not found", func() {
				mockExecutionService.EXPECT().
					GetExecution(gomock.Any(), shareddomain.ID(executionID)).
					Return(maintenanceDomain.MaintenanceExecution{}, maintenance_usecases.ErrMaintenanceExecutionNotFound)
	...
	Context("markCompleted", func() {
		var router *http.ServeMux
		var executionID string
		var completeRequest maintenance_httpapi_internal.MaintenanceExecutionCompleteRequest

		BeforeEach(func() {
			router = http.NewServeMux()
			controller.AddRoutes(router)
			executionID = "execution-123"
			completeRequest = maintenance_httpapi_internal.MaintenanceExecutionCompleteRequest{
		...
		When("execution not found", func() {
			It("should return not found", func() {
				mockExecutionService.EXPECT().
					MarkExecutionCompleted(gomock.Any(), shareddomain.ID(executionID), "user@example.com").
					Return(maintenance_usecases.ErrMaintenanceExecutionNotFound)
