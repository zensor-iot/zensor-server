...
var _ = ginkgo.Describe("MaintenanceExecution Internal Model", func() {
	...
	ginkgo.Context("ToDomain", func() {
		var internalExecution persistenceInternal.MaintenanceExecution

		ginkgo.BeforeEach(func() {
			now := utils.Time{Time: time.Now()}
			internalExecution = persistenceInternal.MaintenanceExecution{
				ID:            "execution-id",
				Version:       1,
	...
	ginkgo.Context("FromMaintenanceExecution", func() {
		var domainExecution maintenanceDomain.MaintenanceExecution

		ginkgo.BeforeEach(func() {
			domainExecution, _ = maintenanceDomain.NewMaintenanceExecutionBuilder().
		...
		ginkgo.When("converting from domain", func() {
			ginkgo.It("should convert basic fields correctly", func() {
				internal := persistenceInternal.FromMaintenanceExecution(domainExecution)
