...
var _ = ginkgo.Describe("MaintenanceExecutionRepository", func() {
	var (
		...
		repo            maintenanceUsecases.MaintenanceExecutionRepository
		ctx             context.Context
	)
	...
	ginkgo.BeforeEach(func() {
		...
		err = orm.AutoMigrate(&maintenancePersistenceInternal.MaintenanceActivity{})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		mockFactory = pubsub.NewMemoryPublisherFactory()

		repo, err = maintenancePersistence.NewMaintenanceExecutionRepository(mockFactory, orm)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	ginkgo.Context("GetByID", func() {
		...
		ginkgo.When("execution exists in database", func() {
			ginkgo.BeforeEach(func() {
				...
				internalExecution := maintenancePersistenceInternal.FromMaintenanceExecution(execution)
		...
		ginkgo.When("execution does not exist", func() {
			ginkgo.It("should return ErrMaintenanceExecutionNotFound", func() {
				...
				gomega.Expect(err).To(gomega.MatchError(maintenanceUsecases.ErrMaintenanceExecutionNotFound))
